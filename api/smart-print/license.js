...
export const LICENSE_RULES=Object.freeze({initialPrice:2000,initialValidityYears:2,renewalPrice:1000,renewalValidityYears:1,transactionFee:200});
export function createLicense(start=new Date()){const issuedAt=new Date(start);const expiresAt=new Date(issuedAt);expiresAt.setFullYear(expiresAt.getFullYear()+2);return {status:'active',issuedAt:issuedAt.toISOString(),expiresAt:expiresAt.toISOString(),price:2000};}
