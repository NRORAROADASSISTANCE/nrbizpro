...
export default async function handler(req,res){
 ...
 try{
  ...
  const planFees={threeYear:3500,lifetime:6000};
  ...
  const expectedPlanAmount=(planFees[plan]+2500)*100;
  ...
  if(b.last_payment_id===razorpay_payment_id){const r=await sql`SELECT id,business,owner,mobile,email,category,gst,status,plan,subscription_ends,pending_plan,pending_amount,last_payment_id FROM businesses WHERE id=${b.id}`;return send(res,200,{verified:true,alreadyProcessed:true,paymentId:razorpay_payment_id,orderId:razorpay_order_id,user:r.rows[0]})}
  const lifetime=plan==='lifetime';
  let ends=null;
  if(!lifetime){const base=b.status==='active'&&b.subscription_ends&&new Date(b.subscription_ends)>new Date()?new Date(b.subscription_ends):new Date();base.setUTCFullYear(base.getUTCFullYear()+3);ends=base.toISOString()}
  await sql`UPDATE businesses SET status='active',plan=${plan},subscription_ends=${ends},pending_plan=null,pending_amount=0,last_payment_id=${razorpay_payment_id},updated_at=now() WHERE id=${b.id}`;
