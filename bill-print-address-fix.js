// NR BizPro — final invoice print layout
(function(){
  'use strict';
  ...
  function patch(){
    if(typeof window.printBill!=='function'||window.printBill.__nrFinalPrint)return;
    ...
    window.printBill=function(id){
      ...
      const businessEmail=s.email||u.email||'';
      const businessGst=s.gst||u.gst||'';
      ...
      const billTime=b.billTime||dt.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:true});
      const discTotal=Number(b.discount)||0;
      ...
      w.document.write(`<html><head><title>${escP(b.invoice)}</title><style>body{font-family:Arial;padding:30px;max-width:760px;margin:auto;font-size:13px;color:#111}.head{display:flex;justify-content:space-between;border-bottom:2px solid #222;padding-bottom:8px}.addr{margin:5px 0 12px;line-height:1.45}.box{border:1px solid #aaa;padding:9px;margin:8px 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:6px}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{padding:7px;border-bottom:1px solid #ddd;text-align:left}.r{text-align:right}.totals{margin-left:auto;width:300px;margin-top:10px}.line{display:flex;justify-content:space-between;padding:3px}.grand{font-size:16px;font-weight:bold;border-top:1px solid #222;margin-top:5px;padding-top:6px}.delivery{border:1px solid #333;padding:12px;margin-top:14px;height:70px}</style></head><body><div class="head"><div><h2>${escP(s.name||u.business||'NR BizPro')}</h2><div class="addr">${escP(businessAddress)}${businessMobile?`<br>Contact: ${escP(businessMobile)}`:''}${businessEmail?`<br>Email: ${escP(businessEmail)}`:''}${businessGst?`<br>GSTIN: ${escP(businessGst)}`:''}</div></div><div><b>${vehicle?'VEHICLE DELIVERY BILL':'TAX INVOICE'}</b><br>${escP(b.invoice)}<br>Date: ${escP(billDate)}<br>Time: ${escP(billTime)}</div></div><div class="box"><b>Customer Details</b><div class="grid"><span>Name: ${escP(b.customer||'Walk-in Customer')}</span><span>Mobile: ${escP(b.mobile||'—')}</span><span style="grid-column:1/-1">Address: ${escP(customerAddress||'—')}</span></div></div>${vehicleHtml}<div class="box"><b>Items</b><table><thead><tr><th>Item / Vehicle</th><th>Type</th><th>Qty</th><th class="r">Rate</th><th class="r">Amount</th></tr></thead><tbody>${rows}</tbody></table></div><div class="totals"><div class="line"><span>Subtotal</span><b>${moneyP(b.subtotal??b.total??0)}</b></div><div class="line"><span>Discount${b.discountType==='percent'&&b.discountValue?` (${escP(b.discountValue)}%)`:''}</span><b>${moneyP(discTotal)}</b></div><div class="line"><span>GST</span><b>${moneyP(b.gstAmount??0)}</b></div><div class="line grand"><span>Net Amount</span><b>${moneyP(b.total)}</b></div><div class="line"><span>Paid</span><b>${moneyP(b.paid??0)}</b></div><div class="line"><span>Due</span><b>${moneyP(b.due??Math.max(0,(+b.total||0)-(+b.paid||0)))}</b></div></div>${vehicle?'<div class="delivery"><b>DELIVERY CONFIRMATION</b><br>Vehicle delivered to customer in good condition.<br>Customer Signature: ____________________ &nbsp; Delivery Date: ____________________</div>':''}</body></html>`);
