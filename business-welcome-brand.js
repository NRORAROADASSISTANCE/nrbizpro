// NR BizPro — dynamic business welcome branding (server-authoritative)
(function(){'use strict';
 ...
 function paint(name){
  ...
  if(!h)return;
  if(name){h.innerHTML='Welcome to <strong>'+esc(name)+'</strong>';
 ...
 async function apply(){
  ...
  if(!name){
   try{const r=await fetch('/api/auth?action=me',{credentials:'include',cache:'no-store',headers:{'Cache-Control':'no-cache'}});const d=await r.json().catch(()=>({}));if(r.ok&&d.user){window.currentUser={...(window.currentUser||{}),...d.user};name=pick(d.user);}}
   catch{}
  }
  if(name)paint(name);
 }
 window.addEventListener('load',()=>setTimeout(apply,100));
 window.addEventListener('authReady',()=>setTimeout(apply,50));
 window.addEventListener('loginSuccess',()=>setTimeout(apply,50));
