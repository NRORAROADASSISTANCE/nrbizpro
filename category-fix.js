...
(function(){
  ...
  function fix(){
    ...
    select.id='suCategory'; select.name='businessCategory'; select.required=true; select.setAttribute('aria-label','Business Category');
    categories.forEach((value,index)=>{const option=document.createElement('option');option.value=value;option.textContent=value;if(index===0)option.selected=true;select.appendChild(option)});
  ...
  function start(){
    ...
    const s=document.createElement('script'); s.src='product-choice.js'; s.defer=true; document.body.appendChild(s);
  }
