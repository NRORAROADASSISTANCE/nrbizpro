...
(function(){
  ...
  async function syncCurrentUser(){
    ...
    try{
      ...
      const local=window.state,cloud=await getCloud();if(cloud?.businessId&&String(cloud.businessId)!==String(u.id))throw new Error('cloud_business_mismatch');if(!cloud.exists){if(hasLocalData(local))await putCloud(local)}else{const localHas=hasLocalData(local),cloudHas=!!(cloud.items?.length||cloud.bills?.length||cloud.customers?.length||cloud.settings?.__nrBizProPurchases?.length||cloud.settings?.__nrBizProExpenses?.length);if(!cloudHas&&localHas&&!localStorage.getItem(key(u.id))){await putCloud(local)}else{const cs=cloud.settings&&typeof cloud.settings==='object'?cloud.settings:{};const cloudState=cloud.state&&typeof cloud.state==='object'&&!Array.isArray(cloud.state)?cloud.state:null;const hasFullState=!!(cloudState&&Object.keys(cloudState).length);const baseState=hasFullState?cloudState:(local||{});window.state={...(baseState||{}),items:Array.isArray(cloud.items)?cloud.items:(Array.isArray(baseState?.items)?baseState.items:[]),bills:Array.isArray(cloud.bills)?cloud.bills:(Array.isArray(baseState?.bills)?baseState.bills:[]),customers:Array.isArray(cloud.customers)?cloud.customers:(Array.isArray(baseState?.customers)?baseState.customers:(local?.customers||[])),purchases:Array.isArray(baseState?.purchases)?baseState.purchases:(Array.isArray(cs.__nrBizProPurchases)?cs.__nrBizProPurchases:(local?.purchases||[])),expenses:Array.isArray(baseState?.expenses)?baseState.expenses:(Array.isArray(cs.__nrBizProExpenses)?cs.__nrBizProExpenses:(local?.expenses||[])),settings:{...(baseState?.settings||local?.settings||{}),...cs}};delete window.state.settings.__nrBizProPurchases;delete window.state.settings.__nrBizProExpenses;if(window.state?.settings&&u){window.state.settings={...window.state.settings,name:u.business,category:u.category,mobile:u.mobile,gst:u.gst||'',address:u.address||'',owner:u.owner,email:u.email};}if(typeof originalSave==='function')originalSave()}}localStorage.setItem(key(u.id),'1');if(typeof window.renderItems==='function')window.renderItems();if(typeof window.renderBills==='function')window.renderBills();if(typeof window.renderCustomers==='function')window.renderCustomers();if(typeof window.updateStats==='function')window.updateStats()}catch(e){console.warn('NR BizPro cloud sync:',e)}finally{syncing=false}}
