(() => {
  ...
  const REG_FEE=3500;
  const PLANS={year3:{label:'3 Years',years:3,fee:3500},lifetime:{label:'Lifetime',years:0,fee:6000}};
  ...
  window.renderAuth=function(mode='login',message=''){
    ...
    if(mode==='signup')el.innerHTML=`<div class="auth-title"><h1>Create your business account</h1><p>Complete registration details before selecting a membership.</p></div><form onsubmit="signup(event)"><div class="auth-grid"><label>Business Name<input id="suBusiness" required></label><label>Owner Name<input id="suOwner" required></label><label>Mobile<input id="suMobile" required inputmode="tel"></label><label>Email<input id="suEmail" required type="email"></label><label>Business Category<select id="suCategory" required><option value="">Select Business Category</option><option>Grocery / General Store</option><option>Footwear / Chappal Shop</option><option>Fertilizer / Agriculture</option><option>Garage / Service Center</option><option>Spare Parts</option><option>EV Two-Wheeler Showroom</option><option>Retail / Supermarket</option><option>Restaurant / Bakery</option><option>Hardware / Building Materials</option><option>Medical / Pharmacy</option><option>Electronics / Mobile</option><option>Clothing / Fashion</option><option>Furniture</option><option>Jewellery</option><option>Stationery / Book Store</option><option>Dairy / Milk Products</option><option>Salon / Beauty Parlour</option><option>Printing / Xerox / Online Services</option><option>Wholesale / Distributor</option><option>Professional Services</option><option>Construction / Building Materials</option><option>Other Business</option></select></label><label>GSTIN <span>(optional)</span><input id="suGst"></label></div><label>Password<input id="suPassword" required minlength="6" type="password"></label><button class="primary auth-btn">Continue to Membership</button></form><div class="auth-switch"><button class="secondary auth-btn" onclick="renderAuth('login')">Back to Login</button></div>`;
    if(mode==='plans')el.innerHTML=`<div class="auth-title"><h1>Activate Membership</h1><p>Registration fee is mandatory. Select one membership plan.</p></div><div class="registration-fee"><span>One-time Registration Fee</span><strong>${money(REG_FEE)}</strong></div><div class="plans">${Object.entries(PLANS).map(([key,p])=>`<button class="plan ${key==='year3'?'featured':''}" onclick="startSubscription('${key}')"><b>${p.label}</b><strong>${money(p.fee)}</strong><span>Membership fee • ${p.years?`${p.years} years`:'Lifetime access'}</span><em>Total payable: ${money(REG_FEE+p.fee)}</em></button>`).join('')}</div>${message?`<div class="notice">${esc(message)}</div>`:''}`;
  ...
  function ensureAuthVisible(){const screen=document.getElementById('authScreen'),el=document.getElementById('authContent');if(screen&&el&&!screen.classList.contains('hidden')&&!el.innerHTML.trim())window.renderAuth('login');}
  ensureAuthVisible();
