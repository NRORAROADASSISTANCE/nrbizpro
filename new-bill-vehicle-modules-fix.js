...
(function(){
  ...
  const EV_RE=/ev.*(?:showroom|service|vehicle)|electric/i;
  ...
  function saveBill(){if(!window.billCart.length)return alert('Add at least one product');const isEV=EV_RE.test(String(window.currentUser?.category||window.currentUser?.business||''));const vehicle=isEV?{model:document.getElementById('nbVehicleModel')?.value.trim()||'',number:document.getElementById('nbVehicleNumber')?.value.trim()||'',chassis:document.getElementById('nbChassisNumber')?.value.trim()||'',engine:document.getElementById('nbEngineNumber')?.value.trim()||'',battery:document.getElementById('nbBatteryNumber')?.value.trim()||'',colour:document.getElementById('nbVehicleColour')?.value.trim()||''}:null;if(isEV&&!vehicle.number&&!vehicle.chassis&&!vehicle.engine)return alert('Enter Vehicle Number or Chassis Number or Engine/Motor Number for EV billing.');let sub=0,gst=0;const items=window.billCart.map(l=>{const i=state.items.find(x=>x.id===l.id),amt=(+i.sell||0)*l.qty;sub+=amt;gst+=amt*(+i.gst||0)/100;if(i.type!=='Service')i.stock=Math.max(0,(+i.stock||0)-l.qty);return {id:i.id,name:i.name,type:i.type,qty:l.qty,price:+i.sell||0,gst:+i.gst||0,amount:amt,businessModule:i.businessModule||i.businessCategory||'' ,vehicleDetails:i.vehicleDetails||null}});const markup=+document.getElementById('nbMarkup')?.value||0,discount=+document.getElementById('nbDiscount')?.value||0,total=Math.max(0,sub+markup-discount+gst);const n=(state.bills||[]).length+1;const bill={id:crypto.randomUUID(),invoice:'INV-'+String(n).padStart(4,'0'),date:new Date().toISOString(),billDate:new Date().toISOString().slice(0,10),billTime:new Date().toTimeString().slice(0,5),customer:document.getElementById('nbCustomer')?.value.trim()||'Walk-in Customer',mobile:document.getElementById('nbMobile')?.value.trim()||'',customerAddress:document.getElementById('nbAddress')?.value.trim()||'',customerGstin:document.getElementById('nbGstin')?.value.trim()||'',items,subtotal:sub,markup,discount,gstAmount:gst,total,businessId:window.currentUser?.id,businessType:window.currentUser?.category,businessCategoryKey:typeof normalizeCategory==='function'?normalizeCategory(window.currentUser?.category||state?.settings?.category):window.currentUser?.category,vehicle,isEV,vehicleDetails:vehicle};state.bills=state.bills||[];state.bills.push(bill);save();closeModal();if(typeof renderBills==='function')renderBills();if(typeof updateStats==='function')updateStats();alert('Bill generated successfully: '+bill.invoice)}
  window.NRVehicleFixToggle=toggleVehicleFields;window.NRVehicleSaveProduct=saveProduct;window.NRVehicleAddBillItem=addBillItem;window.NRVehicleQty=(id,d)=>{const l=window.billCart.find(x=>x.id===id);if(!l)return;const i=state.items.find(x=>x.id===id);l.qty=Math.max(1,Math.min((+i.stock||999999),l.qty+d));renderBill()};window.NRVehicleRemove=id=>{window.billCart=window.billCart.filter(x=>x.id!==id);renderBill()};window.NRVehicleSaveBill=saveBill;window.NRVehicleOpenBill=openBill;window.launchNewBill=openBill;window.openBillModal=openBill;window.openItemModal=openProduct;
})();
