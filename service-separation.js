...
(function(){
  ...
  function render(){
    ...
    const core={
      'Paint Product Management':()=>window.showTab?.('items'),'Plumbing Product Management':()=>window.showTab?.('items'),'Product Management':()=>window.showTab?.('items'),
      'Customer Management':()=>window.showTab?.('customers'),'Customer Sales History':()=>window.showTab?.('customers'),
      'Sales Billing':()=>window.launchNewBill?.(),'Discount Management':()=>window.launchNewBill?.(),
      'Bill History / Reprint':()=>window.showTab?.('bills'),'Stock & Reorder':()=>window.showTab?.('items'),
      'Pipe / Size Management':()=>window.showTab?.('items'),'Pipe / Size / Unit Management':()=>window.showTab?.('items'),
      'Brand / Shade Management':()=>window.showTab?.('items'),'Brand Management':()=>window.showTab?.('items'),'Unit Management':()=>window.showTab?.('items'),
      'Credit / Due Tracking':()=>window.showTab?.('customers'),'Sales Reports':()=>window.showTab?.('bills')
    };
    ...
    document.getElementById('ppFeatureTest').onclick=()=>alert(p.label+' workflow includes '+p.features.length+' modules. Products/Stock, Customers, Sales Billing, Discounts, Credit/Due and Bill History are connected to the main NR BizPro flow.');
  }
  ...
  setInterval(function(){addCategories();removeExtras();render();loadCredit()},3000);
})();
