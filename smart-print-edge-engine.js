/* NR BizPro Smart Print — multi-pass document edge detector with safe fallback. */
(function(){
  function order(p){
  ...
  function candidateFromContours(contours,scale,iw,ih,frame){
    ...
    for(let i=0;i<contours.size();i++){
      ...
      cv.approxPolyDP(cnt,approx,Math.max(1.5,.018*peri),true);
      const area=Math.abs(cv.contourArea(approx));
      if(approx.rows===4 && area>frame*.055 && area<frame*.985 && cv.isContourConvex(approx)){
        const pts=[];
        for(let j=0;j<4;j++){const q=approx.intPtr(j,0);pts.push({x:q[0]/scale,y:q[1]/scale});}
        const p=order(pts);
        ...
        const r=w/h;
        const plausible=(r>=.38&&r<=.98)||(r>=1.02&&r<=2.7);
        const margin=Math.min(...pts.map(q=>Math.min(q.x,q.y,iw-q.x,ih-q.y)));
        if(w>90&&h>90&&plausible&&margin>=0){
          const long=Math.max(w,h),short=Math.min(w,h);
          const edgeBonus=margin>4?1:0;
          const areaScore=area/frame;
          const shapeScore=(short/long>=.35)?1:0;
          out.push({pts:p,area,score:areaScore+edgeBonus*.08+shapeScore*.03});
        }
      }
  ...
  function find(src,mode){
    let gray=null,work=null,blur=null,edge=null,thr=null,kernel=null,contours=null,hier=null;
    try{
      gray=new cv.Mat();cv.cvtColor(src,gray,cv.COLOR_RGBA2GRAY);
      blur=new cv.Mat();cv.GaussianBlur(gray,blur,new cv.Size(5,5),0);
      work=new cv.Mat();
      if(mode==='threshold'){
        cv.adaptiveThreshold(blur,work,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY_INV,31,9);
      }else{
        edge=new cv.Mat();
        cv.Canny(blur,edge,mode==='soft'?12:24,mode==='soft'?70:90);
        work=edge;
      }
    ...
    finally{[gray,work,blur,edge,thr,kernel,contours,hier].forEach(x=>{try{x&&x.delete()}catch{}})}
  }
  function detect(src){
    ...
    try{
      input=cv.imread(src);
      const maxSide=1800,scale=Math.min(1,maxSide/Math.max(input.cols,input.rows));
      ...
      for(const mode of ['normal','soft','threshold']){
        const got=find(small,mode);
        for(const c of got){c.pts=c.pts.map(p=>({x:p.x/scale,y:p.y/scale}));c.area*=1/(scale*scale);all.push(c)}
      }
      if(!all.length)return null;
      // Prefer the largest credible page, but avoid selecting a nearly-full photo frame.
      all.sort((a,b)=>b.score-a.score || b.area-a.area);
      const best=all[0];
      if(best.area/(input.cols*input.rows)>.975)return null;
      return best.pts;
  ...
  function warp(src,q){
    ...
    const pad=Math.max(10,Math.round(Math.min(rawW,rawH)*.018)),w=rawW+2*pad,h=rawH+2*pad;
