/* NR BizPro Smart Print — final output, color, multi-file and same-page print fix. */
(function(){
  ...
  async function pdfPages(f){
    ...
    for(let n=1;n<=doc.numPages;n++){
      const p=await doc.getPage(n),vp=p.getViewport({scale:2}),c=document.createElement('canvas');
  ...
  async function finalLoadPhoto(e){
    ...
    try{
      for(const f of files){
        if(f.type==='application/pdf'||/\.pdf$/i.test(f.name))multiPages.push(...await pdfPages(f));
        else if(f.type.startsWith('image/'))multiPages.push(await read(f));
      }
      if(!multiPages.length)throw new Error('No supported files');
      const q=$('quality');
      if(q){
        q.textContent=passportMode()?'✓ Photo loaded • 8 passport photos will be prepared for 4×6 inch photo paper • original photo untouched.':`✓ ${files.length} file${files.length>1?'s':''} loaded • ${multiPages.length} printable page${multiPages.length>1?'s':''}. Original files remain untouched.`;
  ...
  function installMulti(){
    ...
    input.multiple=true;input.removeAttribute('onchange');input.addEventListener('change',finalLoadPhoto);
    const label=input.closest('.upload');
    if(label){
      const s=label.querySelector('span');if(s)s.textContent='JPG / PNG / PDF • Select multiple files for multi-document printing • originals are never overwritten';
      if(!label.querySelector('[data-multi-note]')){const n=document.createElement('small');n.dataset.multiNote='1';n.style.display='block';n.style.marginTop='6px';n.style.fontWeight='600';n.textContent='📚 Multi-document upload enabled';label.appendChild(n)}
    }
  ...
  function printSamePage(pagesData,pageCss){
    const old=document.getElementById('__nr_print_frame');if(old)old.remove();
    const frame=document.createElement('iframe');frame.id='__nr_print_frame';frame.setAttribute('aria-hidden','true');
    ...
    const pages=pagesData.map(p=>`<div class="p"><img src="${p}"></div>`).join('');
    d.open();
    d.write(`<!doctype html><html><head><title>NR BizPro Smart Print</title><style>html,body{margin:0;padding:0;background:#fff}${pageCss}</style></head><body>${pages}</body></html>`);
    d.close();
    const cleanup=()=>setTimeout(()=>frame.remove(),500);
    frame.onload=()=>setTimeout(()=>{try{frame.contentWindow.focus();frame.contentWindow.print();}catch(e){alert('Printer dialog could not be opened. Please try Print Now again.')}},350);
    frame.contentWindow.addEventListener('afterprint',cleanup,{once:true});
    setTimeout(()=>{if(document.getElementById('__nr_print_frame'))frame.remove()},120000);
  }
  async function printMulti(){
    ...
    let pagesData=multiPages,pageCss;
    if(passportMode()){
      pagesData=Array.from({length:copies},async()=>await passportSheetData());
      pagesData=await Promise.all(pagesData);
      pageCss='@page{size:6in 4in;margin:0}html,body{width:6in;margin:0}.p{width:6in;height:4in;display:flex;align-items:center;justify-content:center;page-break-after:always;overflow:hidden}.p:last-child{page-break-after:auto}.p img{display:block;width:6in;height:4in;max-width:6in;max-height:4in;object-fit:fill;}';
    }else{
      const count=pagesData.length;
      pagesData=Array.from({length:copies},()=>pagesData).flat();
    ...
    printSamePage(pagesData,pageCss);
  }
  ...
  function patchButtons(){
    Array.from(document.querySelectorAll('.actions button')).forEach(b=>{
      const t=(b.textContent||'').trim();
      if(/^Print$/i.test(t)){b.textContent='🖨️ Print Now';b.onclick=printMulti}
      else if(t.includes('Scanner Preview'))b.onclick=previewMulti;
    });
  }
  function ensurePassportPaperOption(){
    ...
    if(passportMode()){
      ...
      if($('typeHint'))$('typeHint').textContent='📸 1-Click Passport Photo: DSLR-style processing → 8 × 35×45 mm → 4×6 inch photo paper (6×4 landscape) → Color → 300 DPI.';
    }
  }
  function boot(){
    installMulti();ensurePassportPaperOption();
    window.__idfbRun=window.runScannerPreview;window.__idfbConfirm=window.confirmScannerPrint;
    window.runScannerPreview=previewMulti;window.confirmScannerPrint=printMulti;window.__finalDirectPrint=printMulti;patchButtons();
    const oldSelect=window.selectType;
    if(typeof oldSelect==='function'&&!oldSelect.__finalOutputFix){
      const s=function(t){const r=oldSelect(t);setTimeout(()=>{ensurePassportPaperOption();window.runScannerPreview=idMode()?idPreview:previewMulti;window.confirmScannerPrint=idMode()?idConfirm:printMulti;window.__finalDirectPrint=idMode()?idConfirm:printMulti;patchButtons()},50);return r};
      s.__finalOutputFix=true;window.selectType=s;
    }
  }
