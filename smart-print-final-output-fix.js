...
(function(){
  ...
  async function finalLoadPhoto(e){
    ...
    try{
      ...
      const q=$('quality');
      if(q){q.textContent=`✓ ${files.length} file${files.length>1?'s':''} loaded • ${multiPages.length} printable page${multiPages.length>1?'s':''}. Original files remain untouched.`;q.classList.remove('hidden')}
  ...
  function printMulti(){
    ...
    const copies=Math.max(1,+$('copies')?.value||1),paper=$('paper')?.value||'A4',mode=$('mode')?.value||'Color';
  ...
  function previewMulti(){
    ...
    if(!multiPages.length)return alert('Upload one or more WhatsApp documents, photos or PDFs first.');
    const copies=Math.max(1,+$('copies')?.value||1),body=$('previewBody');if(!body)return;
  ...
  function patchButtons(){
    Array.from(document.querySelectorAll('.actions button')).forEach(b=>{const t=(b.textContent||'').trim();if(/^Print$/i.test(t)){b.textContent='🖨️ Print Now';b.onclick=printMulti}else if(t.includes('Scanner Preview'))b.onclick=previewMulti});
  }
  function boot(){
    installMulti();
    window.__idfbRun=window.runScannerPreview;window.__idfbConfirm=window.confirmScannerPrint;
    window.runScannerPreview=previewMulti;window.confirmScannerPrint=printMulti;patchButtons();
    const oldSelect=window.selectType;
    if(typeof oldSelect==='function'&&!oldSelect.__finalOutputFix){
      const s=function(t){const r=oldSelect(t);setTimeout(()=>{window.runScannerPreview=idMode()?idPreview:previewMulti;window.confirmScannerPrint=idMode()?idConfirm:printMulti;patchButtons()},50);return r};
