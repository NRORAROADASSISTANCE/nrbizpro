/* NR BizPro Smart Print — Passport FINAL v35: real 4x6 sheet preview + same-page print. */
(function(){
  'use strict';
  ...
  document.addEventListener('click',e=>{
    const b=e.target.closest?.('.types button[data-type="passport"]'); if(b) passportSelected=true;
    const non=e.target.closest?.('.types button:not([data-type="passport"])'); if(non) passportSelected=false;
  },true);
  ...
  async function makeSheet(){
    ...
    if(!window.smartPassportPremium?.sheet)throw new Error('Passport DSLR engine is not ready. Refresh once and try again.');
  ...
  async function passportPreview(){
    ...
    }catch(err){console.error(err);resultCanvas=null;body.innerHTML='<div class="ai-badge">⚠️ '+String(err.message||'Passport preview failed.')+' Original photo is unchanged.</div>'}
  }
