...
(function(){
  ...
  async function xeroxTarget(file){
    ...
    // Build a shadow reference from the brighter/right paper area. This treats the left camera shadow as illumination, not document content.
    ...
    bx.filter='blur(1.5px)';bx.drawImage(c,0,0,gw,gh);var bd=bx.getImageData(0,0,gw,gh).data;
    function lum(j){return .2126*bd[j]+.7152*bd[j+1]+.0722*bd[j+2]}
    var rowRef=new Float32Array(gh);
    for(var gy=0;gy<gh;gy++){
      var vals=[];for(var gx=Math.floor(gw*.58);gx<gw;gx++)vals.push(lum((gy*gw+gx)*4));
      vals.sort(function(a,b){return a-b});rowRef[gy]=vals[Math.floor(vals.length*.78)]||220;
    }
    ...
    for(var y=0;y<h;y++){
      ...
      for(var x=0;x<w;x++){
        ...
        var target=Math.max(215,Math.min(246,refAt(gy)+12));
        // Strong multiplicative illumination correction: remove the dark left/top camera shade while preserving ink.
        var gain=target/Math.max(48,local);gain=Math.max(.90,Math.min(2.70,gain));
        var i=(y*w+x)*4,r=d[i],g=d[i+1],b=d[i+2],l=.2126*r+.7152*g+.0722*b;
        // Only very dark pixels are protected as ink. Mid-tone paper in shadow is lifted aggressively.
        var protect=l<48?.12:l<85?.12+.58*(l-48)/37:l<125?.70+.30*(l-85)/40:1;
        var eg=1+(gain-1)*protect;
        r=Math.min(255,r*eg);g=Math.min(255,g*eg);b=Math.min(255,b*eg);
        // Neutralize residual warm/green cast on bright paper.
        var nl=.2126*r+.7152*g+.0722*b;
        if(nl>150){var neutral=Math.min(1,(nl-150)/90)*.22;r=r*(1-neutral)+nl*neutral;g=g*(1-neutral)+nl*neutral;b=b*(1-neutral)+nl*neutral}
        var contrast=nl<120?1.06:1.02;
  ...
  async function renderPreview(){
    ...
    try{var url=await xeroxTarget(f);show('<div class="ai-badge">✓ Smart Xerox Target — strong camera-shadow removal, paper whitening and text protection applied.</div><div class="preview-sheet"><p><b>Document • 1 page • '+copies+' copy/copies</b></p><img src="'+url+'" alt="Smart Xerox target preview" style="max-width:100%;height:auto;display:block;margin:auto"></div>')}catch(e){console.error('Smart Xerox preview',e);show('<div class="warn">Preview processing failed. Please upload the original WhatsApp file again.</div>')}
  }
