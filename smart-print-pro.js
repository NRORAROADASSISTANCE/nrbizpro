...
(function(){
  ...
  function cleanDocument(src,mode='Black & White'){
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){const i=(y*w+x)*4,g=.2126*d[i]+.7152*d[i+1]+.0722*d[i+2],background=Math.max(70,sampleBg(x,y));let n=clamp(g*(235/background));n=clamp(205+(n-205)*1.12);if(mode==='Black & White')n=n>190?255:n<90?0:Math.round(255*((n-90)/100));d[i]=d[i+1]=d[i+2]=n;d[i+3]=255}
