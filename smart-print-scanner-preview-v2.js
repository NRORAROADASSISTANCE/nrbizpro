...
(function(){
  ...
  function clarityClean(c){
    ...
    const sw=Math.max(48,Math.min(150,Math.round(w/22))),sh=Math.max(48,Math.min(150,Math.round(h/22))),small=document.createElement('canvas');small.width=sw;small.height=sh;
    ...
    // Low-frequency illumination field: removes shadows without erasing printed detail.
    const blur=new Float32Array(field.length),radius=Math.max(4,Math.round(Math.min(sw,sh)/18));
    for(let y=0;y<sh;y++)for(let x=0;x<sw;x++){
      let sum=0,n=0;for(let yy=Math.max(0,y-radius);yy<=Math.min(sh-1,y+radius);yy++)for(let xx=Math.max(0,x-radius);xx<=Math.min(sw-1,x+radius);xx++){sum+=field[yy*sw+xx];n++;}blur[y*sw+x]=sum/n;
    }
    const vals=Array.from(field).filter(v=>v>45&&v<250).sort((a,b)=>a-b),p50=vals.length?vals[Math.floor(vals.length*.50)]:180,p92=vals.length?vals[Math.floor(vals.length*.92)]:235;
    const target=Math.max(205,Math.min(242,p92));
    for(let y=0;y<h;y++){
      ...
      for(let x=0;x<w;x++){
        ...
        const a=blur[y0*sw+x0]*(1-tx)+blur[y0*sw+x1]*tx,b=blur[y1*sw+x0]*(1-tx)+blur[y1*sw+x1]*tx,bg=Math.max(70,a*(1-ty)+b*ty);
        const k=(y*w+x)*4,g=.2126*d[k]+.7152*d[k+1]+.0722*d[k+2];
        // Gentle local illumination correction. Dark ink is protected; paper is lifted.
        let corrected=g*Math.pow(target/bg,.68);
        corrected=.72*g+.28*corrected;
        if(g<90)corrected=.88*g+.12*corrected;
        corrected=Math.max(0,Math.min(255,corrected));
        // Mild contrast around the document midpoint for crisp Xerox output.
        let n=128+(corrected-128)*1.08;
  ...
  function run(){
    ...
    const body=$('previewBody');$('preview').classList.remove('hidden');body.innerHTML='<div class="ai-badge">⏳ Scanner processing — safe edge detection, perspective correction and print-grade shadow normalization…</div>';
    setTimeout(async()=>{try{
      const original=canvasFromImage(await img(await read(f))),cropped=edgeCrop(original);resultCanvas=clarityClean(cropped);
      body.innerHTML='<div class="ai-badge">✓ Clean Print — document edges protected • perspective corrected only when confident • uneven shadows normalized • paper clarity improved • original untouched.</div><div class="preview-sheet"><p><b>Processed document • 1 page</b></p><img src="'+resultCanvas.toDataURL('image/jpeg',.97)+'" alt="Processed uploaded document"></div>';
