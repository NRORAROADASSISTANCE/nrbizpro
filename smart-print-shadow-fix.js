/* NR BizPro Smart Print — Xerox shadow correction v11
...
function fixShadow(src,mode){
 ...
 const tw=Math.max(100,Math.min(220,Math.round(w/18))),th=Math.max(120,Math.min(260,Math.round(h/18)));
 ...
 function blur(a,rad){
  const rr=Math.max(1,Math.round(rad)),tmp=new Float32Array(a.length),out=new Float32Array(a.length);
  for(let y=0;y<th;y++){
   let sum=0;
   for(let k=-rr;k<=rr;k++){const xx=Math.max(0,Math.min(tw-1,k));sum+=a[y*tw+xx]}
  ...
  for(let xx=0;xx<tw;xx++){
   let sum=0;
   for(let k=-rr;k<=rr;k++){const yy=Math.max(0,Math.min(th-1,k));sum+=tmp[yy*tw+xx]}
   for(let y=0;y<th;y++){
 ...
 const refs=[];
 for(let y=0;y<th;y++)for(let xx=Math.floor(tw*.72);xx<tw;xx++){const v=field[y*tw+xx];if(v>105)refs.push(v)}
 const ref=clamp(pct(refs,.55),185,230);
 for(let y=0;y<h;y++){
  ...
  for(let xx=0;xx<w;xx++){
   ...
   const edge=smooth(clamp((.94-xn)/.94,0,1));
   const deficit=clamp((ref-local)/Math.max(80,ref*.72),0,1);
   const i=(y*w+xx)*4,r=d[i],g=d[i+1],b=d[i+2],L=.2126*r+.7152*g+.0722*b;
   let protect;if(L<28)protect=.08;else if(L<50)protect=.18+.42*(L-28)/22;else if(L<90)protect=.60+.40*(L-50)/40;else protect=1;
   const strength=deficit*edge*.92*protect;
   // Multiplicative luminance correction keeps the original colour balance better than RGB whitening.
   const ratio=clamp(ref/Math.max(45,local),1,2.15);
...
function install(){
 ...
 if(original.__nrShadowV11)return true;
 const wrapped=async function(){
  ...
  try{
   ...
   body.innerHTML=`<div class="ai-badge">✓ Xerox Clean — left camera shadow corrected, original colour preserved and document details protected.</div><div class="preview-sheet"><p><b>Document • Page 1 • ${copies} copy/copies</b></p><img src="${page}" alt="Print preview page 1"></div>`;
   document.getElementById('preview').classList.remove('hidden');
  }catch(e){console.error(e);return original.apply(this,arguments)}
 };
 wrapped.__nrShadowV11=true;window.previewPrint=wrapped;
 const baseConfirm=window.confirmPrint;
 if(typeof baseConfirm==='function'&&!baseConfirm.__nrShadowV11){
  ...
  confirmWrapped.__nrShadowV11=true;window.confirmPrint=confirmWrapped;
 }
 return true;
}
