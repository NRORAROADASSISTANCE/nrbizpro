/* NR BizPro Smart Print — Xerox shadow correction v14
   Tiny refinement over v13: preserve the current look and gently reduce the remaining
   central camera shadow without changing document colour or fine details. */
(function(){
'use strict';
...
function fixShadow(src,mode){
 ...
 for(let y=0;y<h;y++){
  ...
  for(let xx=0;xx<w;xx++){
   ...
   const middle=smooth(clamp((.62-Math.abs(xn-.50))/.62,0,1));
   ...
   const strength=deficit*(leftEdge*.90+middle*.10)*.96*protect;
...
function install(){
 ...
 if(original.__nrShadowV14)return true;
 ...
 wrapped.__nrShadowV14=true;window.previewPrint=wrapped;
 const baseConfirm=window.confirmPrint;
 if(typeof baseConfirm==='function'&&!baseConfirm.__nrShadowV14){
  ...
  confirmWrapped.__nrShadowV14=true;window.confirmPrint=confirmWrapped;
 }
 return true;
}
