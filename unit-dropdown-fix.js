...
function addUnitToExisting(){const s=getState();if(!s?.items)return;let changed=false;s.items.forEach(i=>{if(!i.unit){i.unit='pcs';changed=true}});if(changed)saveUnits()}
...
function install(){
  ...
  window.addItem=function(){
    ...
    saveUnits();closeModal();if(typeof window.renderItems==='function')window.renderItems();if(typeof updateStats==='function')updateStats();
  };
  window.searchBillProducts=function(){
    addUnitToExisting();const s=getState(),input=getBillInput(),box=getBillBox();if(!input||!box)return;const q=(input.value||'').trim().toLowerCase();if(!q){box.innerHTML='';return}
...
function bindBillSearch(){
  ...
  input.addEventListener('keydown',e=>{if(e.key!=='Enter')return;e.preventDefault();const q=input.value.trim().toLowerCase(),s=getState(),items=s?.items||[],i=items.find(x=>String(x.barcode||'').toLowerCase()===q);if(i){window.NRBizProAddBillItem(i.id)}});
}
