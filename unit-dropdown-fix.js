...
window.addItem=function(){
  ...
  const item={id:crypto.randomUUID(),name,barcode,type:document.getElementById('mType')?.value||'Product',unit:document.getElementById('mUnit')?.value||'pcs',cost:+document.getElementById('mCost')?.value||0,margin:+document.getElementById('mMargin')?.value||0,marginType:document.getElementById('mMarginType')?.value||'percent',sell:+document.getElementById('mSell')?.value||0,gst:+document.getElementById('mGst')?.value||0,stock:+document.getElementById('mStock')?.value||0,businessCategory:window.currentUser?.category||window.state?.settings?.category||'General Business'};
  window.state.items.push(item);saveUnits();closeModal();renderItems();updateStats();
};
window.renderItems=function(){
  addUnitToExisting();const tb=document.getElementById('itemTable');if(!tb)return;const items=(window.state?.items||[]);if(!items.length){tb.innerHTML='<tr><td colspan="9" class="empty">No products for this business yet.</td></tr>';return}
  tb.innerHTML=items.map(i=>`<tr><td><b>${esc(i.name)}</b></td><td>${esc(i.barcode||'—')}</td><td>${esc(i.type||'Product')}</td><td>${esc(getUnit(i))}</td><td>${money(i.cost)}</td><td><b>${money(i.sell)}</b></td><td>${i.gst||0}%</td><td>${i.stock||0}</td><td><button class="secondary" onclick="deleteItem('${i.id}')">Delete</button></td></tr>`).join('');
};
window.searchBillProducts=function(){
  ...
  const items=(window.state?.items||[]);const found=items.filter(i=>String(i.name||'').toLowerCase().includes(q)||String(i.barcode||'').toLowerCase()===q).slice(0,8);
